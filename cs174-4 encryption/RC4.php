...
{
    $S_arr = array(); // int array
    $S_arr_size = 256;

    $K_arr = array(); // byte array
    $key_len = strlen($key); // N
    $input_arr = array();

    for ( $i = 0; $i < strlen($key); $i++ ) {
        echo substr($key,$i,1)."<br>";
        $key[$i] = ord(substr($key,$i,1));
        echo $key[$i]."<br>";
    }

    for ( $i = 0; $i < strlen($plainText); $i++ ) {
        $input_arr[$i] = ord($plainText{$i});
    }

    // fill STable
    for ($i = 0; $i < $S_arr_size; $i++) {
        $S_arr[$i] = $i;
        $K_arr[$i] = mb_strcut($key,($i%$key_len),1);
    }
    //
    $j = 0;
    for ($i = 0; $i < $S_arr_size; $i++) {
        // j = (j + S[i] + K[i]) mod 256
        $j = ($j + $S_arr[$i]+$K_arr[$i])%$S_arr_size;
        // swap
        $tmp = $S_arr[$i];
        $S_arr[$i] = $S_arr[$j];
        $S_arr[$j] = $tmp;
